...
function randomizeAvatarAndClothes() {
    ...
    const ontopHairImagesRand = document.querySelectorAll('.ontop-image');
    ...
    const randomHairIndex = getRandomIndex(ontopHairImagesRand.length);
    ...
    ontopHairImagesRand.forEach((image, index) => {
        if (index === randomHairIndex) {
            image.style.display = 'block';
        } else {
            image.style.display = 'none';
        }
